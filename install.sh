...
linkMin() {
    ...
    if ln -s $(pwd)/.gitconfig $(pwd)/.gitconfig; then
    ...
    if ln -s $(pwd)/VScode/snippets $(pwd)/.config/Code/User/snippets; then
    ...
    rm -rf $(pwd)/.tmux.conf &>/dev/null
    if ln -s $(pwd)/.tmux.conf $(pwd)/.tmux.conf; then
    ...
    rm -rf $(pwd)/.zshrc &>/dev/null
    rm -rf $(pwd)/.fzf.zsh &>/dev/null
    rm -rf $(pwd)/.oh-my-zsh/themes/cute.zsh-theme &>/dev/null
    if ln -s $(pwd)/.zshrc $(pwd)/.zshrc; then
        echo "[install.sh]-- .zshrc linked"
    else
        echo "[install.sh]-- .zshrc linking failed"
        failedLinks+=(".zshrc")
    fi
    if ln -s $(pwd)/.fzf.zsh $(pwd)/.fzf.zsh; then
        echo "[install.sh]-- .fzf.zsh linked"
    else
        echo "[install.sh]-- .fzf.zsh linking failed"
        failedLinks+=(".fzf.zsh")
    fi
    if ln -s $(pwd)/cute.zsh-theme $(pwd)/.oh-my-zsh/themes/cute.zsh-theme; then
    ...
    mkdir -p $(pwd)/GitHub/Uni/ &>/dev/null
    mkdir -p $(pwd)/.ssh &>/dev/null
    mv $(pwd)/.ssh/config $(pwd)/.ssh/config.bak &>/dev/null
    if ln -s $(pwd)/Uni/sshConfig $(pwd)/.ssh/config; then
