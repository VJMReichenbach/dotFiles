...
```bash
sudo pacman -S --noconfirm 7zip bat bitwarden btop cargo-binstall clang dust evince eza file-roller fwupd fzf ghostty git-delta gum gvfs-mtp gvfs lazygit libmtp mtpfs network-manager-applet neovim nextcloud-client nushell qt5-wayland qt6-wayland ripgrep rust starship tar tealdeer thunar thunar-archive-plugin thunar-volman thunderbird unrar unzip zellij
```
